...
class Consumer {
  ...
  public:
    ...
    // ID of the Consumer (initialized from memory adr.)
    int ConsumerID;
...
template <typename Arg>
Consumer<Arg>::Consumer(int _id, _callbackType _callback, Arg _arg, long _interval) {
  enabled = true;
  onRun(_callback);
  _cached_next_run = 0;
  last_run = 0;
...
void Consumer<Arg>::run() {
  ...
  if (_onRun != NULL && _callbackArgument != NULL) {
